 ...
 ******************************************************/
$RETURNURL = G5_SHOP_URL.'/danalpay/dbank/CPCGI.php'; 
...
$REQ_DATA["BYPASSVALUE"] = "a=b;c=d;";

/**************************************************
 * 고객 정보
**************************************************/
$REQ_DATA["USERNAME"]  = iconv('utf-8', 'euc-kr', $od['od_name']); // 구매자 이름 (euc-kr 로 변환처리)
$REQ_DATA["USERID"]    = $member['mb_id'] ? $member['mb_id'] : 'guest'; // 사용자 ID
$REQ_DATA["USEREMAIL"] = $od['od_email']; // 소보법 email수신처
$REQ_DATA["USERAGENT"] = "PC"; // 2019-03-18 : 가상계좌일경우.
if (is_mobile()) { 
...
//$RES_DATA = CallDanalBankExec($REQ_DATA, false); //curl_init() 함수 이용이 불가능할때, curl 바이너리를 호출(curl 설치 필요)
