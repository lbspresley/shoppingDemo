            ...
            <div class="table-list-eb">
                ...
                <table class="table">
                    <tbody>
                        ...
                        <?php
                        // wetoz : danalpay
                        if(!isset($default['de_danalpay_cpid'])) {
                            sql_query(" ALTER TABLE `{$g5['g5_shop_default_table']}`
                                            ADD  `de_danalpay_cpid` VARCHAR( 20 ) NOT NULL ,
                                            ADD  `de_danalpay_cryptokey` VARCHAR( 255 ) NOT NULL,
                                            ADD  `de_danalpay_ispreotbill` TINYINT( 4 ) NOT NULL DEFAULT '0',
                                            ADD  `de_danalpay_itemcode` VARCHAR( 255 ) NOT NULL
                                            ; ", true);
                        }

                        if(!isset($default['de_danalpay_hp_cpid'])) {
                            sql_query(" ALTER TABLE `{$g5['g5_shop_default_table']}`
                                            ADD  `de_danalpay_hp_cpid` VARCHAR( 20 ) NOT NULL ,
                                            ADD  `de_danalpay_hp_cryptokey` VARCHAR( 100 ) NOT NULL,
                                            ADD  `de_danalpay_dbank_cpid` VARCHAR( 20 ) NOT NULL ,
                                            ADD  `de_danalpay_dbank_cryptokey` VARCHAR( 100 ) NOT NULL,
                                            ADD  `de_danalpay_vbank_cpid` VARCHAR( 20 ) NOT NULL ,
                                            ADD  `de_danalpay_vbank_cryptokey` VARCHAR( 100 ) NOT NULL
                                            ; ", true);
                        }

                        if (!function_exists('openssl_encrypt')) {
                            echo '<script>'.PHP_EOL;
                            echo 'alert("openssl extension 이 설치되어 있지않습니다.\n서버 관리자에게 문의해 주십시오.");'.PHP_EOL;
                            echo '</script>'.PHP_EOL;
                        }
                        if (!function_exists('curl_init')) {
                            echo '<script>'.PHP_EOL;
                            echo 'alert("Curl extension 이 설치되어 있지않습니다.\n서버 관리자에게 문의해 주십시오.");'.PHP_EOL;
                            echo '</script>'.PHP_EOL;
                        }
                        ?>

                        <style>.danalpay_info_fld th.table-form-th {background-color: #fffff6;}</style>

                        <tr class="pg_info_fld danalpay_info_fld">
                            <th class="table-form-th">다날페이 (신용카드)</th>
                            <td>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <i class="icon-prepend text-width">CPID</i>
                                    <input type="text" name="de_danalpay_cpid" value='<?php echo $default['de_danalpay_cpid']?>'>
                                </label>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <i class="icon-prepend" style="width:100px">CRYPTOKEY</i>
                                    <input type="text" name="de_danalpay_cryptokey" value='<?php echo $default['de_danalpay_cryptokey']?>'>
                                </label>
                                <div class="note margin-bottom-10"><strong>Note:</strong> 다날페이 가맹점관리자 페이지 : https://cp.teledit.com/login/<br>CPID, CRYPTOKEY 는 다날 계약담당자에게 문의 바랍니다.<br />실서비스 정보와 테스트정보가 다르므로 실서비스를 위해서는 반드시 교체가 필요합니다.<br />테스트용 CPID, CRYPTOKEY 가 별도로 발급되므로 테스트를 위해서는 별도로 발급받은 값을 입력해주세요.</div>
                            </td>
                        </tr>
                        <tr class="pg_info_fld danalpay_info_fld">
                            <th class="table-form-th">다날페이 (휴대폰)</th>
                            <td>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <i class="icon-prepend text-width">CPID</i>
                                    <input type="text" name="de_danalpay_hp_cpid" value='<?php echo $default['de_danalpay_hp_cpid']?>'>
                                </label>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <i class="icon-prepend" style="width:100px">CRYPTOKEY</i>
                                    <input type="text" name="de_danalpay_hp_cryptokey" value='<?php echo $default['de_danalpay_hp_cryptokey']?>'>
                                </label>
                                <div class="note margin-bottom-10"><strong>Note:</strong> CPID, CPPWD 값을 결제수단에 관계없이 모두 동일한 값을 부여받았다면 모두 동일하게 입력해주세요.</div>
                            </td>
                        </tr>
                        <tr class="pg_info_fld danalpay_info_fld">
                            <th class="table-form-th">다날페이 (계좌이체)</th>
                            <td>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <i class="icon-prepend text-width">CPID</i>
                                    <input type="text" name="de_danalpay_dbank_cpid" value='<?php echo $default['de_danalpay_dbank_cpid']?>'>
                                </label>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <i class="icon-prepend" style="width:100px">CRYPTOKEY</i>
                                    <input type="text" name="de_danalpay_dbank_cryptokey" value='<?php echo $default['de_danalpay_dbank_cryptokey']?>'>
                                </label>
                            </td>
                        </tr>
                        <tr class="pg_info_fld danalpay_info_fld">
                            <th class="table-form-th">다날페이 (가상계좌)</th>
                            <td>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <i class="icon-prepend text-width">CPID</i>
                                    <input type="text" name="de_danalpay_vbank_cpid" value='<?php echo $default['de_danalpay_vbank_cpid']?>'>
                                </label>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <i class="icon-prepend" style="width:100px">CRYPTOKEY</i>
                                    <input type="text" name="de_danalpay_vbank_cryptokey" value='<?php echo $default['de_danalpay_vbank_cryptokey']?>'>
                                </label>
                            </td>
                        </tr>
                        <tr class="pg_info_fld danalpay_info_fld">
                            <th class="table-form-th">ItemCode</th>
                            <td>
                                <label for="de_kcp_mid" class="input form-width-250px">
                                    <input type="text" name="de_danalpay_itemcode" value='<?php echo $default['de_danalpay_itemcode']?>'>
                                </label>
                                <div class="note margin-bottom-10"><strong>Note:</strong> 휴대폰 결제를 위하여 다날에서 제공한 ItemCode 입력</div>
                            </td>
                        </tr>
                        <!-- wetoz : danalpay -->
