...
.shop-order-form .order-table th {width:90px;padding:5px 10px;background:none;text-align:left}
...
function forderform_check(f)
{
    ...
    if( jQuery(f).triggerHandler("form_sumbit_order_"+form_order_method) !== false ) {
        ...
        // 결제정보설정
        <?php if($default['de_pg_service'] == 'danal') { ?>
        ...
        <?php if($default['de_pg_service'] == 'danalpay') { // wetoz : danalpay - pc ?>
        switch(settle_method)
        {
            case "계좌이체":
